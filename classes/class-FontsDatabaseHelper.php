...
class FontsDatabaseHelper {
	...
	public static function get_usable_fonts(){
		...
		if(!self::$usable_fonts){
        	...
        	self::$usable_fonts = $wpdb->get_results('SELECT * FROM ' . $wpdb->prefix . BK_USABLE_FONTS_DATABASE . ' ORDER BY id DESC');
		}
	...
	public static function get_custom_elements(){
		...
		if(!self::$custom_elements){
        	...
        	self::$custom_elements = $wpdb->get_results('SELECT e.id, u.name, e.font_id, e.font_weight, e.custom_elements, e.important FROM ' . $wpdb->prefix . BK_ELEMENTS_DATABASE . ' as e LEFT OUTER JOIN ' . $wpdb->prefix . BK_USABLE_FONTS_DATABASE . ' as u ON ' . ' e.font_id = u.id ORDER BY e.font_id DESC');
		}
	...
	public static function get_usable_font($name){
        ...
        return $wpdb->get_row('SELECT * FROM ' . $wpdb->prefix . BK_USABLE_FONTS_DATABASE . ' WHERE name = "' . $name . '" ORDER BY id DESC');
